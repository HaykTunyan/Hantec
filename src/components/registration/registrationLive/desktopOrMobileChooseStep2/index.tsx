...
const DesktopOrMobileChooseStep2 = ({setSelectedTab, lineWidth}: IStepsInfo) => {
    const [shutftiLink, setShutftiLink] = useState<string>("");

    const {t} = useTranslation("onboarding");
    const addressAuto: any = t("main_event.address_verify.address_auto", {returnObjects: true});

    useEffect(() => {
        getTokenForShufti("address")
            .then(res => {
                if (res && res.data) {
    ...
    return (
        <div
            ...
            <div
                className="mobile:max-w-full tablet:px-6 mobile:px-4 w-full flex flex-col gap-6 tablet:mb-10">
                <div className="max-w-[362px] mx-auto flex flex-col gap-6">
                    ...
                    <span className="text-12_14 text-grey-tertiary">
                        {addressAuto.manual_text}
                        <span className="font-medium"> {addressAuto.manual_btn}</span>
                    </span>
                        <button
                            className={"btnPrim"}
                            style={{flex: "0 0 auto"}}
                ...
                {/*        className="btnSec"*/}
                {/*        disabled={disabled}*/}
                {/*        onClick={() => setSelectedTab("continue-here2")}*/}
                {/*    >*/}
                {/*        Continue here*/}
                {/*    </button>*/}
                {/*    <button*/}
...
export default DesktopOrMobileChooseStep2;
